...
<head>
    <title>Management | Breezing Bolt</title>
...
<div class="min-h-85vh bg-gradient-to-b from-gray-800 to-black">
    ...
    <table class="min-w-full border-collapse block md:table">
        ...
        <tr class="bg-gray-800 border border-grey-700 shadow-lg shadow-gray-800/50 md:border-none block md:table-row">
            <td class="p-2 md:border md:border-gray-700 text-left text-white block md:table-cell"><span class="inline-block w-1/3 md:hidden font-bold">Name</span><span id="name-<%= user.getId()%>"><%= user.getFirst_name() + " " + user.getLast_name()%></span></td>
            <td class="p-2 md:border md:border-gray-700 text-left text-white block md:table-cell"><span class="inline-block w-1/3 md:hidden font-bold">User Name</span><span id="username-<%= user.getId()%>"><%= user.getUsername()%></span></td>
            <td class="p-2 md:border md:border-gray-700 text-left text-white block md:table-cell"><span class="inline-block w-1/3 md:hidden font-bold">Email Address</span><span id="email-<%= user.getId()%>"><%= user.getEmail()%></span></td>
            <td class="p-2 md:border md:border-gray-700 text-left text-white block md:table-cell">
                <span class="inline-block w-1/3 md:hidden font-bold">Actions</span>
                <button id="edit-<%= user.getId()%>" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 border border-blue-500 rounded transition ease-in-out duration-150" onclick="onEditClick(event)">Edit</button>
                <button id="delete-<%= user.getId()%>" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 border border-red-500 rounded transition ease-in-out duration-150" onclick="onClickDelete(event)">Delete</button>
            </td>
        </tr>
        <% } %>
        </tbody>
